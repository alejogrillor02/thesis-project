...
for str1 in "${model_strings[@]}"; do
	for str2 in "${set_strings[@]}"; do
		./evaluate.py "${MODEL_DIR}/model_${str1}/set_${str2}" "${TRAINDATA_DIR}/model_${str1}/set_${str2}/${str1}_${str2}_test.txt" $folds "${OUTPUT_DIR}" && echo "Done evaluating $str model for $str2 set."
	done
done